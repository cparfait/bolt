...
import { seDesinscrireParLien } from "@/lib/actions/liens-courriel";
import type { ActionState } from "@/lib/actions/types";
import { btnDanger, btnSecondary } from "@/components/ui";
import { SubmitButton } from "@/components/submit-button";

/**
 * Libérer sa place après des absences répétées, en deux temps.
 *
 ...
 * évite à l'autre de perdre une place qu'il comptait reprendre à la rentrée
 * des vacances.
 */
export function BoutonDesinscription({
  inscriptionId,
  signature,
  activite,
  occupee,
  ...
  signature: string;
  activite: string;
  /** L'inscription tient-elle encore une place ? Faux : il n'y a rien à libérer. */
  occupee: boolean;
}) {
  const [state, action] = useActionState<ActionState, FormData>(
    seDesinscrireParLien,
    null,
  );
  ...
  if (!occupee) {
    return (
      <p className="mt-4 flex items-start gap-2 rounded-xl bg-slate-50 px-3 py-2.5 text-sm text-slate-500">
        ...
        Vous n&apos;occupez plus de place sur ce créneau — elle a déjà été
        libérée, ou votre inscription a changé depuis l&apos;envoi du message.
      </p>
    );
  }
  ...
  return (
    <form action={action} className="mt-4 space-y-3">
      ...
      <p className="text-sm text-slate-500">
        Vous avez manqué plusieurs séances de suite. Si vos disponibilités ont
        changé, libérez votre place : elle repartira aussitôt à la personne
        suivante sur la liste d&apos;attente. Si c&apos;est un contretemps
        passager, ne faites rien : votre place vous attend.
      </p>
